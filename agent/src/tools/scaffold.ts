...
/**
 * The boilerplate's package.json is copied verbatim, which means the repository
 * root's own scripts travel with it. Those that drive the agent workspace are
 * meaningless in a generated app — `npm --prefix ./agent` points at a directory
 * deliberately left out of BOILERPLATE_CONTENTS — so they would ship broken.
 *
 * Matching the path rather than the script name keeps this honest: a new script
 * added at the root is pruned because of what it does, not because someone
 * remembered to add its name to a list here.
 */
const AGENT_WORKSPACE_REFERENCE = /\.\/agent\b/;

async function pruneAgentScripts(targetRoot: string): Promise<readonly string[]> {
  ...
  const removed = Object.keys(scripts).filter((name) =>
    AGENT_WORKSPACE_REFERENCE.test(scripts[name] ?? ""),
  );
  if (removed.length === 0) return [];

  for (const name of removed) delete scripts[name];
...
export async function scaffold(options: ScaffoldOptions): Promise<ScaffoldResult> {
  ...
  await pruneAgentScripts(targetRoot);

  await writeFile(
    join(targetRoot, MARKER_FILE),
    `Generated by car-inventory-agent from the provided boilerplate.\n` +
